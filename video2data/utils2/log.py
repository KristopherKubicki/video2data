...
def shot_recap(prev_shot):
  ...
  #print('\t[%s] Length: %.02fs, %d frames' % (shottime,float((prev_shot['end'] - prev_shot['start']) / prev_shot['fps']),prev_shot['end'] - prev_shot['start']))
  #print('\t[%s] Detect Method: %s ' % (shottime,data['frame_type']))
...
def scene_recap(prev_scene):
  ...
  #print('[%s] Length: %.02fs, %d frames' % (scenetime,prev_scene['length'],frame['frame'] - last_scene + frame['break_count']))
